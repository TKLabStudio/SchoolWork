...
class Ball(pg.sprite.Sprite):
    ...
    def __init__(self, sp, srx, sry, radium, color):
        ...
        self.image.convert()
        pg.draw.circle(self.image, (0,0,0), (-100,-100), radium, 0)
...
class Pad(pg.sprite.Sprite):
    def __init__(self):
        pg.sprite.Sprite.__init__(self)
        self.image = pg.image.load("pad.png")  #滑板圖片
        self.image.convert()
        self.rect = self.image.get_rect()
...
def gameover(message): 
    ...
    if message == 1:
        bggo = "fail.png"
        pg.mixer.Sound("longlive.wav").play()
    else:
        bggo = "win.png"
        pg.mixer.Sound("Kaohsiung.wav").play()
    image = pg.image.load(bggo)
    image.convert()
    bg.blit(image, (0,0))
    screen.blit(bg, (0,0))
...
soundhit = pg.mixer.Sound("Fadacai.wav")  #碰到磚塊音效
soundpad = pg.mixer.Sound("Fadacai.wav")  #碰到滑板音效
#背景
...
ball = Ball(15, 2, 350, 30, (255,255,255)) #建立球體
allsprite.add(ball)  #加入全部角色群組
pad = Pad()          #建立滑板球物件
allsprite.add(pad)   #加入全部角色群組

#建立磚塊
#for row in range(0, 1):          #5列方塊
#    for column in range(0, 1):  #每列15磚塊
#        brick = Brick((254,0,0), column * 40 + 1, row * 15 + 1)   #位置為40*15

for row in range(0, 9):          #5列方塊
    for column in range(0, 15):  #每列15磚塊
        brick = Brick((254,0,0), column * 40 + 1, row * 15 + 1)   #位置為40*15  
        if column == 0 or column == 1 or column == 2 or column == 3 or column == 4:  
            brick = Brick((0,0,149), column * 40 + 1, row * 15 + 1)
        if row == 5 or row == 6 or row == 7 or row == 8:
            brick = Brick((254,0,0), column * 40 + 1, row * 15 + 1)   #位置為40*15
        if (column == 1 and row == 1) or (column == 1 and row == 2) or (column == 1 and row == 3) or  (column == 2 and row == 1) or (column == 2 and row == 2) or (column == 2 and row == 3) or (column == 3 and row == 1) or (column == 3 and row == 2) or (column == 3 and row == 3) or (column == 0 and row == 2 or (column == 2 and row == 0) or (column == 2 and row == 4) or (column == 4 and row == 2)): 
            brick = Brick((200,200,200), column * 40 + 1, row * 15 + 1)   #位置為40*15
            
        bricks.add(brick)     #加入磚塊角色群組
        allsprite.add(brick)  #加入全部角色群組
        
clock = pg.time.Clock()        
downmsg = "Click Left Button to Start!"  #起始訊息
